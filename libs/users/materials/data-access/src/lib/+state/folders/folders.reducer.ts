...
const reducer = createReducer(
  ...
  on(FoldersActions.loadFoldersFailure, (state) => ({ ...state, status: 'error' as const })),
  ...
  on(FoldersActions.addFolderFailure, (state) => ({ ...state, status: 'error' as const })),
  ...
  on(FoldersActions.removeFolderFailure, (state) => ({ ...state, status: 'error' as const })),
  //________________________________Get Folder
  on(FoldersActions.getFolderSuccess, (state, { folder }) => foldersAdapter.setOne( folder, { ...state, status: 'loaded' as const }))
);
