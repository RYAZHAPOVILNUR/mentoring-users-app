...
import { CreateMaterialDTO, MaterialDTO, MaterialsEntity, MaterialsFacade } from '@users/materials/data-access';
...
export class CreateMaterialsButtonComponent {
  ...
  openDialog(type: 'video' | 'audio' | 'default' | 'pdf'): void {
    ...
    dialogRef.afterClosed().subscribe((result: MaterialsEntity) => {
      this.materialsFacade.addMaterial({ ...result, folder_id: this.folder_id, type: type });
    });
  }
}
