...
export class DetailUsersCardComponent implements OnInit {
  ...
  private onAddStoryPointSuccess: onSuccessSPonCbType = () =>
  ...
  onAddStoryPoints() {
    ...
    this.addStoryPoints.emit({
      ...
      onSuccessAddSP: this.onAddStoryPointSuccess,
    });
  }
  ...
  onOpenEditMode() {
    this.vm.editMode = true;  // Включаем режим редактирования
    // Включаем все поля формы
    Object.keys(this.formGroup.controls).forEach((controlName) => {
      this.formGroup.controls[controlName as keyof typeof this.formGroup.controls].enable();
    });
  }
