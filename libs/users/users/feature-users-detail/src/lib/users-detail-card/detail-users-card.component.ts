...
export class DetailUsersCardComponent implements OnInit {
  ...
  set vm(vm: DetailUsersCardVm) {
    ...
    if (vm.user) {
      ...
      this.formGroupStoryPoints.patchValue({
        inputStoryPoints: vm.user.totalStoryPoints,
      });
    }
  ...
  public inputStoryPointsToggled = false;
  public inputStoryPointsDisabled = true;

  public formGroupStoryPoints = new FormGroup({
    inputStoryPoints: new FormControl(
      { value: 0, disabled: this.inputStoryPointsDisabled },
      [Validators.required, Validators.pattern(/^(10|[0-9])$/)]
    ),
  });
  ...
  @Output() changeStoryPoints = new EventEmitter<{
    user: CreateUserDTO;
    onEditSuccessSP: onSuccessStoryPointsCbType;
  }>();
  @ViewChild('snackbar') snackbarTemplateRef!: TemplateRef<any>;
  @ViewChild('snackbarStoryPoints')
  snackbarStoryPointsTemplateRef!: TemplateRef<any>;
  ...
  private onEditSuccessStoryPoints: onSuccessStoryPointsCbType = () =>
  ...
  onToggleInput() {
    this.inputStoryPointsToggled = !this.inputStoryPointsToggled;
    this.inputStoryPointsDisabled = !this.inputStoryPointsDisabled;
    if (!this.inputStoryPointsDisabled) {
      this.formGroupStoryPoints.enable();
    } else {
      this.formGroupStoryPoints.disable();
    }
  }

  onChangeStoryPoints() {
    if (this.formGroupStoryPoints.valid) {
      this.changeStoryPoints.emit({
        user: {
          ...
          educationStatus: 'trainee',
          totalStoryPoints:
            this.formGroupStoryPoints.value.inputStoryPoints || 0,
        },
        onEditSuccessSP: this.onEditSuccessStoryPoints,
      });
      this.formGroupStoryPoints.disable();
      this.inputStoryPointsDisabled = true;
      this.inputStoryPointsToggled = false;
    }
  }
}
