...
export class UsersDetailComponent {
  ...
  public readonly editStoryPointsMode$: Observable<boolean> = this.store.pipe(
    select(selectQueryParam('editStoryPoints')),
    map((param) => param === 'true')
  );
  public readonly errors$: Observable<UsersErrors | null> =
    this.usersFacade.errors$;
  ...
  public onEditUser(
    ...
    this.router.navigate(['/admin/users', this.user.id], {
      queryParams: { edit: false, editStoryPoints: false },
    });
  }

  public onEditUserStoryPoints(
    totalStoryPoints: number,
    onSuccessCb: onSuccessEditionCbType
  ) {
    this.usersFacade.updateUserStoryPoints(
      totalStoryPoints,
      this.user.id,
      onSuccessCb
    );
  ...
  onOpenEditMode() {
    this.router.navigate(['admin/users', this.user.id], {
      queryParams: { edit: true, editStoryPoints: true },
    });
  }

  onOpenEditStoryPointsMode() {
    this.router.navigate(['admin/users', this.user.id], {
      queryParams: { edit: false, editStoryPoints: true },
    });
  }
