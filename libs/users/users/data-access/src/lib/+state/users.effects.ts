...
export const editStorypointsUser = createEffect(
  () => {
    const actions$ = inject(Actions);
    const apiService = inject(ApiService);
    ...
    return actions$.pipe(
      ofType(UsersActions.editStorypointsUser),
      ...
      switchMap(({ user, onSuccessCb }) =>
        apiService.post<UsersDTO, CreateUserDTO>(`/users/${user.id}`, user).pipe(
          ...
          map(({ userData }) => UsersActions.editStorypointsUserSuccess({ userData })),
          catchError((error) => {
            console.error('Error', error);
            return of(UsersActions.editStorypointsUserFailed({ error }));
          })
        )
      )
    );
