...
export const updateTotalStoryPoints = createEffect(
  () => {
    ...
    return actions$.pipe(
      ofType(UsersActions.updateUserTotalStoryPoints),
      withLatestFrom(usersEntities$),
      filter(([{ id }, usersEntities]) => Boolean(usersEntities[id])),
      ...
      switchMap(({ user, onSuccessCb }) =>
        apiService
          ...
          .pipe(
            ...
            tap(({ onSuccessCb }) => onSuccessCb()),
            map(({ userData }) =>
              UsersActions.updateUserTotalStoryPointsSuccess({ userData })
            ),
            catchError((error) => {
              console.error('Error', error);
              return of(
                UsersActions.updateUserTotalStoryPointsFailed({ error })
              );
            })
          )
      )
