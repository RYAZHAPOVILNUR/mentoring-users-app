...
export const filteredUsers = createSelector(selectAllUsers, usersFilterSelector, (users: UsersEntity[], { name }: { name: string }) => 
  users.filter((user: UsersEntity) => user.name.toLowerCase().includes(name))
);
