...
export const selectFilteredUsers = createSelector(usersFilterSelector, selectAllUsers, (usersFilter, alUsers) => {
  if (!usersFilter.name) return alUsers
  return alUsers.filter((user) => user.name.includes(usersFilter.name));
});
 // filter for users 
