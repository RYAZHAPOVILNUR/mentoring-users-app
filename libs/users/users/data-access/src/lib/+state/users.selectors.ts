...
export const selectFilteredUsers = createSelector( usersFilterSelector, selectAllUsers, 
  (usersFilter,users) =>
  usersFilter.name 
  ? users.filter(user => user.name.toLowerCase().startsWith(usersFilter.name.toLowerCase())) 
  : users
);